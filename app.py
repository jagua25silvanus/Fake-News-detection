...
def api_key_page():
    ...
    with col2:
        ...
        st.markdown("<small>Get free key at <a href='https://console.groq.com' target='_blank'>console.groq.com</a></small>", unsafe_allow_html=True)
...
def load_models():
    ...
    return embedder, nli_pipe, all_chunks, index
...
{{"formal_claim": "...", "year": "...", "location": "...", "topic": "..."}}"""
    ...
    try:
        ...
        return {"formal_claim": raw_claim, "year": None, "location": "India", "topic": "other"}
...
def get_nli_verdict(claim, evidence_text, nli_pipe):
    result = nli_pipe(
        claim,
        candidate_labels=["supported by evidence", "contradicted by evidence", "uncertain"],
        hypothesis_template="Based on climate data, this claim is {}"
    )
    scores = dict(zip(result["labels"], result["scores"]))
    true_score = scores.get("supported by evidence", 0)
    fake_score = scores.get("contradicted by evidence", 0)
    unc_score  = scores.get("uncertain", 0)
    return true_score, fake_score, unc_score

def multi_vote(claim, evidence_chunks, nli_pipe):
    all_scores = []
    display = []
    for chunk in evidence_chunks:
        t, f, u = get_nli_verdict(claim, chunk["text"], nli_pipe)
        all_scores.append((t, f, u))
        if t > f and t > u:    v = "TRUE"
        elif f > t and f > u:  v = "FAKE"
        else:                   v = "UNCERTAIN"
        display.append((v, max(t, f, u), chunk))
    avg_true = sum(s[0] for s in all_scores) / len(all_scores)
    avg_fake = sum(s[1] for s in all_scores) / len(all_scores)
    avg_unc  = sum(s[2] for s in all_scores) / len(all_scores)
    if avg_true > 0.60:
        verdict, confidence = "TRUE", avg_true
    elif avg_fake > 0.25:
        verdict, confidence = "FAKE", avg_fake
    elif avg_unc > 0.35:
        verdict, confidence = "UNCERTAIN", avg_unc
    else:
        verdict, confidence = "UNCERTAIN", avg_unc
    return verdict, confidence, display
...
def main():
    ...
    with st.sidebar:
        st.markdown("## 🌦️ IMD Fake News Detector")
        st.markdown("---")
    ...
    if page == "🔍 Claim Verification":
        ...
        with st.spinner("Loading models... (first time takes 2-3 mins)"):
            embedder, nli_pipe, all_chunks, index = load_models()

        st.markdown("### 💡 Try these examples:")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("India received above normal rainfall in 2019"):
                st.session_state.claim = "India received above normal rainfall in 2019"
            if st.button("India never had any monsoon rainfall in its entire history"):
                st.session_state.claim = "India never had any monsoon rainfall in its entire history"
        with col2:
            if st.button("2023 monsoon was delayed by 3 weeks in India"):
                st.session_state.claim = "2023 monsoon was delayed by 3 weeks in India"
            if st.button("Cyclone Amphan hit West Bengal in 2020"):
                st.session_state.claim = "Cyclone Amphan hit West Bengal in 2020"

        claim = st.text_area(
            "Enter your climate claim:",
            value=st.session_state.get("claim", ""),
            placeholder="e.g. India received above normal rainfall in 2019",
            height=100
        )
        ...
        if st.button("✅ Check Claim", type="primary"):
            if claim.strip():
                with st.spinner("Analyzing claim..."):
                    ...
                    final_verdict, confidence, verdicts = multi_vote(formal, evidence, nli_pipe)
        ...
        st.title("💬 Feedback")
        st.markdown("We would love to hear your thoughts!")
        name = st.text_input("Your Name (optional)")
        rating = st.slider("⭐ Rate this app", 1, 5, 3)
        stars = "⭐" * rating
        st.markdown(f"Your rating: **{stars} ({rating}/5)**")
        category = st.selectbox("Feedback about:", [
            "Overall Experience", "Claim Verification Accuracy",
            "UI / Design", "Speed / Performance", "Other"
        ])
        comment = st.text_area("Your Comments", height=150)
        if st.button("📨 Submit Feedback", type="primary"):
            if comment.strip():
                with open("feedback.txt", "a") as f:
                    f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Name: {name or 'Anonymous'} | Rating: {rating}/5 | Category: {category} | Comment: {comment}\n")
